...
/**
 * コメントから「N号 X cm」パターンを正規表現で抽出する。
 * 対応例:
 *   「1号：3cm」「2号 5cm」「3号:4センチ」「1号3cm」
 *   「1. 1号：3cm」（行頭の箇条書き番号は無視）
 *   「１号：３cm」（全角数字）
 * 同じ号が複数回出てきた場合は最後の値を採用。
 */
export function extractHeightsFromComment(comment: string): Record<number, number> {
  const text = normalizeDigits(comment);
  const pattern = /([1-3])\s*号[\s::：]*(\d+(?:\.\d+)?)\s*(?:cm|センチ|ｃｍ)/gi;
...
### title（記事タイトル）:
- **「Day XX -」「Day XX：」などの接頭辞は絶対につけない**（日付・日数番号はタイトルに含めない）
- 内容を一言で表すキャッチーな日本語タイトルのみ
